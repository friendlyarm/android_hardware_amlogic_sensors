...
static const float sLuxValues[5] = {
    3500.0,
    1500.0,
    800.0,
    500.0,
        ...
        if (fd>=0) {
            ...
            if (!strcmp(name, "compass")) {
                ...
                *akm_fd = fd;
                ...
                LOGV("using %s (name=%s)", devname, name);
                *p_fd = fd;
            } else if (!strcmp(name, "light_sensor")) {
                LOGV("using %s (name=%s)", devname, name);
                *l_fd = fd;
            } else
                close(fd);
...
static native_handle_t* control__open_data_source(struct sensors_control_context_t *dev)
{
    ...
    handle->data[0] = open_mpld_socket();
...
static int data__data_open(struct sensors_data_context_t *dev, native_handle_t* handle)
{
    ...
    dev->events_fd[2] = -1;
#endif
		dev->events_fd[2] = dup(handle->data[1]);
    dev->events_fd[3] = dup(handle->data[0]);
