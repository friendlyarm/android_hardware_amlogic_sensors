...
/******************************************************************************
 * $Id: mlsl_linux.c 4161 2010-11-24 20:51:08Z mcaramello $
 *****************************************************************************/
...
#define MLCAL_FILE    "\\Storage Card\\cal.bin"
#define MLCFG_ID      (0x01020304L)
#define MLCFG_FILE    "\\Storage Card\\cfg.bin"
...
tMLError MLSLReadCfg( unsigned char *cfg, unsigned int len )
{
    ...
    if (((unsigned int)cfg[3] << 24 | cfg[2] << 16 | cfg[1] << 8 | cfg[0]) 
        != MLCFG_ID) {
...
tMLError MLSLReadCal(unsigned char *cal, unsigned int len)
{
    ...
    /* MLCAL_ID not used 
    if (((unsigned int)cal[3] << 24 | cal[2] << 16 | cal[1] << 8 | cal[0]) 
        != MLCAL_ID) {
        return ML_ERROR_ASSERTION_FAILURE;
    }
    */
