		  ...
		  struct ext_slave_platform_data *pdata)
{
	...
	result =
	    MLSLSerialWrite(mlsl_handle, pdata->address, 1, dummyData);
	ERROR_CHECK(result);
	...
	result =
	    MLSLSerialRead(mlsl_handle, pdata->address, dummyRegister, 6,
			   rawData);
	ERROR_CHECK(result);
	xoffset =
	...
	/* CAL data read */
	result =
	    MLSLSerialRead(mlsl_handle, pdata->address, dummyRegister, 9,
			   calData);
	ERROR_CHECK(result);
	/* Config register - CAL register read - "110 01 000" */
	dummyData[0] = YAS_REG_CONFR | 0x08;
