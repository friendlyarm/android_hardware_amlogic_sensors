...
test_result test_open(void) 
{
    ...
    res = MLSLSerialOpen(g_port, &mlsl_handle);
...
test_result test_single_read(void)
{
    ...
    res = MLSLSerialRead(mlsl_handle, g_mpu_slaveaddr, 0x00, 1, &data);
...
test_result test_register_dump(void) 
{
    ...
    for(ii=0; ii<NUM_OF_MPU_REGISTERS; ii++) {
        if (ii == MPUREG_FIFO_R_W || ii == MPUREG_MEM_R_W) {
            ...
            res = MLSLSerialRead(mlsl_handle, g_mpu_slaveaddr, ii, 1, &data);
...
test_result test_burst_wr(void)
{
    ...
    // reset MPU
    MLSLSerialWriteSingle(mlsl_handle, g_mpu_slaveaddr, 
                          MPUREG_PWR_MGM, 0x80);
...
test_result test_bypass(void) 
{
    ...
    MLSLSerialWriteSingle(mlsl_handle, g_mpu_slaveaddr, MPUREG_USER_CTRL, usr_ctrl);
    MLOSSleep(5);
...
test_result test_accel_sanity(void) 
{
    ...
    RECORD_ERROR(ML_ERROR, "Accel test not implemlented");
    RETURN_SUCCESS(r);
}
